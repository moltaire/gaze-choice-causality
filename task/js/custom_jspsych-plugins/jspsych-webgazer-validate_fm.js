  ...
  plugin.trial = function (display_element, trial) {
    ...
    function drawValidationPoint_CenterOffsetMode(x, y) {
      return `<div class="validation-point" style="width:${
        trial.point_size
      }px; height:${trial.point_size}px; border-radius:${
        trial.point_size
      }px; border: 1px solid ${trial.point_border_color}; background-color: ${
    ...
    function drawRawDataPoint_PercentMode(target_x, target_y, dx, dy) {
      var color =
        Math.sqrt(dx * dx + dy * dy) <= trial.roi_radius ? "#afa" : "#faa";
    ...
    function calculateGazeCentroid(gazeData) {
      var x_diff_m = gazeData.reduce(function (
        accumulator,
        currentValue,
        index
      ) {
      ...
      var y_diff_m = gazeData.reduce(function (
        accumulator,
        currentValue,
        index
      ) {
      ...
      var median_distance = median(
        gazeData.map(function (x) {
          return Math.sqrt(
            Math.pow(x.dx - x_diff_m, 2) + Math.pow(x.dy - y_diff_m, 2)
          );
        })
      );
    ...
    function validation_done() {
      trial_data.samples_per_sec = calculateSampleRate(
        trial_data.raw_gaze
      ).toFixed(2);
      for (var i = 0; i < trial.validation_points.length; i++) {
        trial_data.percent_in_roi[i] = calculatePercentInROI(
          trial_data.raw_gaze[i]
        );
        trial_data.average_offset[i] = calculateGazeCentroid(
          trial_data.raw_gaze[i]
        );
      }
